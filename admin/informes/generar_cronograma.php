            ...
            <div class="col">
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                        ...
                        // Ejemplo de uso
                        //$fechaEntrevista = "2023-09-25 08:00:00"; // Fecha de inicio de la programación (casi al inicio del día)
                        //$duracionEntrevista = 30; // Duración de cada entrevista en minutos
                        ...
                        foreach ($cur as $result) {
                            ...
                            // Muestra la fecha y hora de entrevista según el cronograma generado
                            echo '<td>' . array_shift($cronogramaEntrevistas) . '</td>';
                            echo '</tr>';
                        }
                        ...
                        function generarCronograma($fecha, $duracion)
                        {
                            ...
                            $horarioInicioManana = new DateTime('08:00');
                            $horarioFinManana = new DateTime('13:00');
                            $horarioInicioTarde = new DateTime('14:15');
                            $horarioFinTarde = new DateTime('16:45');

                            $cronograma = array();

                            while (count($cronograma) < $duracion) {
                                if (($fechaInicio < $horarioInicioManana || $fechaInicio >= $horarioFinManana)
                                    && ($fechaInicio < $horarioInicioTarde || $fechaInicio >= $horarioFinTarde)
                                ) {
                                    $fechaInicio->add(new DateInterval('P1D'));
                                    $fechaInicio->setTime(8, 0);
                                } else {
                                    $cronograma[] = $fechaInicio->format('Y-m-d H:i:s'); // Formato de fecha y hora
                                    $fechaInicio->modify("+$duracion minutes");
                                }
                            }
                            return $cronograma;
                        }
